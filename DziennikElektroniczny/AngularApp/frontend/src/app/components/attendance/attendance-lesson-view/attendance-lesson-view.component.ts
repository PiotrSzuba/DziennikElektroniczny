...
export class AttendanceLessonViewComponent implements OnInit {
  ...
  members: Map<number, StudentGroupMemberViewModel> = new Map<
    number,
    StudentGroupMemberViewModel
  >();
  attendances: Map<number, AttendanceViewModel> = new Map<
    number,
    AttendanceViewModel
  >();
  tableData: Map<number, AttendanceElement> = new Map<
    number,
    AttendanceElement
  >();
  ...
  constructor(
    ...
    private choosePersonToAnswerService: ChoosePersonToAnswerService
  ) {}
  ...
  private loadTableData() {
    ...
    this.studentGropMemberService
      .getStudentGroupMembers(this.subject.studentsGroupId)
      .subscribe((response) => {
        response.forEach((element) => {
          this.members.set(
            element.studentPersonId,
            new StudentGroupMemberViewModel(
              element.id,
        ...
        if (this.attendances.has(studentId)) {
          ...
          if (attendance?.id) {
            let request = this.attendanceService.updateStudentAttendance(
              ...
              studentId,
              wasPresent,
              attendance.absenceNote
            );
            requests.push(request);
          }
        } else {
          let request = this.attendanceService.createStudentAttendance(
            this.lesson.id,
            studentId,
            wasPresent
          );
  ...
  generatePersonToAnswer() {
    let presence: number[] = [];
    this.wasPresent.forEach((v) => {
      presence.push(v);
    });
    this.choosenNumber = this.choosePersonToAnswerService.choosePerson(
      this.members.size,
      presence
    );
  }
  ifMarked(data: any) {
    if (data == this.choosenNumber) return 'choosen';
    return '';
  }
}
